...
int DataFileIO::unpack_data_m1(vars_t *v)
{
    while (v->processed_size < v->input_size) {
        ...
        while (subchunks--) {
            ...
            if (data_length) {
                ...
                int64_t fix_overflow =
                    ((v->pack_block_start[2] << 16) | (v->pack_block_start[1] << 8) | v->
                     pack_block_start[0]);
                v->bit_buffer =
                    (fix_overflow << v->bit_count) | (v->bit_buffer & ((1 << v->bit_count) - 1));
            }
    ...
    write_buf(v->output, &v->output_offset, &v->decoded[v->dict_size],
              v->window - &v->decoded[v->dict_size]);
    return 0;
}
...
uint32_t DataFileIO::input_bits_m1(vars_t *v, short count)
{
    ...
    while (count--) {
        if (!v->bit_count) {
            ...
            v->bit_buffer =
                (v->pack_block_start[1] << 24) | (v->pack_block_start[0] << 16) | (b2 << 8) | b1;
...
void DataFileIO::write_decoded_byte(vars_t *v, uint8_t b)
{
    ...
    *v->window++ = b;
    v->unpacked_crc_real =
        crc_table[(v->unpacked_crc_real ^ b) & 0xFF] ^ (v->unpacked_crc_real >> 8);
}
