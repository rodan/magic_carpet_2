...
unsigned char *BitmapIO::CreateBitBitmapFileHeader(int fileHeaderSize, int infoHeaderSize,
...
unsigned char *BitmapIO::CreateBitBitmapInfoHeader(int infoHeaderSize, int width, int height,
                                                   int bytesPerPixel)
{
...
void BitmapIO::WriteImageBufferAsImageBMP(const char *path, int width, int height,
                                          uint8_t *ptrPalette, uint8_t *ptrBuffer)
{
    ...
    unsigned char *fileHeader =
        CreateBitBitmapFileHeader(BitmapIO::BITMAP_FILE_HEADER_SIZE,
                                  BitmapIO::BITMAP_INFO_HEADER_SIZE, height, stride);
    fwrite(fileHeader, 1, BitmapIO::BITMAP_FILE_HEADER_SIZE, imageFile);

    unsigned char *infoHeader =
        CreateBitBitmapInfoHeader(BitmapIO::BITMAP_INFO_HEADER_SIZE, width, height,
                                  BitmapIO::TRUECOLOR_BYTES_PER_PIXEL);
...
void BitmapIO::WriteRGBAImageBufferAsImageBMP(const char *path, int width, int height,
                                              uint8_t *ptrPalette, uint8_t *ptrBuffer)
{
    ...
    unsigned char *infoHeader =
        CreateBitBitmapInfoHeader(BitmapIO::BITMAP_INFO_HEADER_SIZE, width, height,
                                  BitmapIO::RGBA_BYTES_PER_PIXEL);
    fwrite(infoHeader, 1, BitmapIO::BITMAP_INFO_HEADER_SIZE, imageFile);

    int i;
    for (i = 0; i < height; i++) {
        fwrite(truColorBuffer + (i * widthInBytes), BitmapIO::RGBA_BYTES_PER_PIXEL, width,
               imageFile);
        fwrite(padding, 1, paddingSize, imageFile);
    }
    delete[]truColorBuffer;
...
void BitmapIO::WritePaletteAsImageBMP(const char *path, int numColors, uint8_t *ptrPalette)
{
    ...
    unsigned char *fileHeader =
        CreateBitBitmapFileHeader(BitmapIO::BITMAP_FILE_HEADER_SIZE,
                                  BitmapIO::BITMAP_INFO_HEADER_SIZE, 100, stride);
    fwrite(fileHeader, 1, BitmapIO::BITMAP_FILE_HEADER_SIZE, imageFile);

    unsigned char *infoHeader =
        CreateBitBitmapInfoHeader(BitmapIO::BITMAP_INFO_HEADER_SIZE, numColors, 100,
                                  BitmapIO::TRUECOLOR_BYTES_PER_PIXEL);
    fwrite(infoHeader, 1, BitmapIO::BITMAP_INFO_HEADER_SIZE, imageFile);
