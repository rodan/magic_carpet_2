...
void VGA_Init(Uint32 /*flags */ , int width, int height, bool maintainAspectRatio, int displayIndex)
{
    ...
    if (!inited) {
        ...
        if (SDL_Init(SDL_INIT_VIDEO) < 0) {
            ...
            {
                m_window =
                    SDL_CreateWindow(default_caption, SDL_WINDOWPOS_UNDEFINED,
                                     SDL_WINDOWPOS_UNDEFINED, width /*dm.w */ , height /*dm.h */ ,
                                     test_fullscr);
            } else {
                ...
                m_window =
                    SDL_CreateWindow(default_caption, display.x, display.y, display.w, display.h,
                                     test_fullscr);
            }
            ...
            m_gamePalletisedSurface =
                SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 24,
                                     redMask, greenMask, blueMask, alphaMask);

            m_gamePalletisedSurface =
                SDL_ConvertSurfaceFormat(m_gamePalletisedSurface, SDL_PIXELFORMAT_INDEX8, 0);
            ...
            m_gameRGBASurface =
                SDL_ConvertSurfaceFormat(m_gameRGBASurface, SDL_PIXELFORMAT_RGB888, 0);
...
void SetPalette(SDL_Color *colours)
{
    ...
    if (m_gamePalletisedSurface) {
        SDL_SetPaletteColors(m_gamePalletisedSurface->format->palette, m_currentPalletColours, 0,
                             256);
...
void Draw_debug_matrix0()
{
    ...
    for (int i = 0; i < 16; i++)
        for (int j = 0; j < 16; j++) {
            ...
            SDL_FillRect(m_gamePalletisedSurface, &dstrect,
                         i * 16 + j /*SDL_MapRGB(screen->format, i*16+j, 0, 0) */ );
        }
...
void Draw_letterToBuffer(int letter_number, int pozx, int pozy, uint8_t *buffer, char fontSize)
{
    ...
    for (int yy = 0; yy < dstrect.h; yy++)
        for (int xx = 0; xx < dstrect.w; xx++) {
            ...
            switch (fontSize) {
            case 'S':
                buffer[(dstrect.y + yy) * m_gamePalletisedSurface->w + (dstrect.x + xx)] =
                    m_smallFontBuffer[srcx + (srcy * 128)];
                break;
            default:
                buffer[(dstrect.y + yy) * m_gamePalletisedSurface->w + (dstrect.x + xx)] =
                    m_fontBuffer[srcx + (srcy * 256)];
                break;
            }
        }
};
...
void VGA_Draw_stringXYtoBuffer(const char *wrstring, int x, int y, uint8_t *buffer, char fontSize)
{
    ...
    for (uint32_t i = 0; i < strlen(wrstring); i++) {
        if (wrstring[i] == '\n') {
            ...
            Draw_letterToBuffer(wrstring[i], x + (loclastpoz % textwidth) * fontSizePixels,
                                y + (loclastpoz / textwidth) * fontSizePixels, buffer, fontSize);
            loclastpoz++;
        }
    }
}
...
bool handleSpecialKeys(const SDL_Event & event)
{
    ...
    if ((event.key.keysym.sym == SDLK_RETURN) && (event.key.keysym.mod & KMOD_ALT)) {
        ...
        SDL_SetWindowGrab(m_window, settingWindowGrabbed ? SDL_TRUE : SDL_FALSE);
        std::cout << "Set window grab to " << (settingWindowGrabbed ? "true" : "false") << std::
            endl;
        specialKey = true;
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_JOYHATMOTION:
            if (event.jhat.which == gpc.controller_id) {
                // actual hat data is being read via gamepad_poll_data()
...
void VGA_Blit(Uint8 *srcBuffer)
{
    ...
    if (m_iOrigh != m_gamePalletisedSurface->h) {
        ...
        m_gamePalletisedSurface =
            SDL_CreateRGBSurface(SDL_SWSURFACE, m_iOrigw, m_iOrigh, 24,
                                 redMask, greenMask, blueMask, alphaMask);
    ...
    if (srcBuffer)
        memcpy(m_gamePalletisedSurface->pixels, srcBuffer,
               m_gamePalletisedSurface->h * m_gamePalletisedSurface->w);
