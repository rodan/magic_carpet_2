...
void alsound_update(void)
{
	...
	for (i = OPENAL_C_SZ; i>0; i--) {
		if (alc[i-1].state == AL_PLAYING) {
			...
			ret = alGetError();
			if (ret != AL_NO_ERROR) {
				Logger->error("error during alDeleteSources: {} for i {}  state {}", alsound_get_error_str(ret), i-1, alc[i-1].state);
			}
			alc[i-1].state = 0;
			alc[i-1].size = 0;
			//Logger->info("freed {}", i);
...
void alsound_update_source(type_event_0x6E8E* entity, axis_3d* position)
{
	...
	if (ent) {
		if (entity->play_ch == -1) {
		    ...
		    alSource3f(alc[entity->play_ch].alSource, AL_POSITION, position->x, position->y, position->z);
...
/// \return play channel index
int16_t alsound_play(const int16_t chunk_id, Mix_Chunk * mixchunk, const uint16_t loops, al_ssp_t *ssp, const uint16_t flags)
{
	int16_t i;
	int16_t cache_ch = -1;
	int16_t play_ch = -1;
	...
	for (i = OPENAL_CC_SZ; i>0; i--) {
		if (alcc[i-1].id == chunk_id) {
			...
			if ( (uint32_t) alcc[i-1].size != mixchunk->alen) {
				Logger->info("cache miss!  new {} cached {}  cache_ch {}", mixchunk->alen, alcc[i-1].size, i-1);
