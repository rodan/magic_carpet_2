...
shadow_myNCB myNCBtoShadow(myNCB from)
{
    ...
    for (int i = 0; i < 16; i++)
        to.ncb_callName_10[i] = from.ncb_callName_10[i];
    for (int i = 0; i < 16; i++)
        to.ncb_name_26[i] = from.ncb_name_26[i];
...
std::string Pack_Message(uint32 message, shadow_myNCB locNCB, int32_t locIndex, int32_t port =
                         -1, const char *data = nullptr, int size_of_data = 0)
{
...
bool setListen(myNCB *locNCB)
{
    ...
    for (int i = 0; i < clientConnection.size(); i++)
        if (memcmp
            (locNCB->ncb_name_26, clientConnection[i]->ncb_name_26,
             sizeof(clientConnection[i]->ncb_name_26))) {
...
namespace MyNetworkLib {
 ...
 public:
    ...
    NetworkClass::NetworkClass(bool iam_server, std::string host, int port, int serverPort,
                               bool IAmServer) {
        clPort = port;
    ...
    void NetworkClass::SendGiveIPMessage(asio::ip::udp::endpoint sender, int port) {
        ...
        SendToClient(Pack_Message
                     (MESS_SERVER_GIVE_IP, nullNCB, -1, port, sender.address().to_string().c_str(),
                      1 + strlen(sender.address().to_string().c_str())),
                     sender.address().to_string(), port);
    };
    ...
    void NetworkClass::Handler() {
        while (HandleHandlerOn) {
            ...
            if (!receiveServerAddName) {
                shadow_myNCB nullNCB;
                nullNCB.ncb_command_0 = 254;
                SendToServer(Pack_Message
                             (MESS_CLIENT_SERVER_NAME_ADDED, nullNCB, GetNextIndex(), clPort));
                ...
                case 0x95:{    //RECEIVE(opposite send)//fix it!!!!!!!!!!!!!!!!!!!
                        if (GetRecCount() > 0) {
                            ...
                            if ((unpacked_message.message != MESS_SERVER_SEND)
                                ...
                                AddRecMess(Pack_Message
                                           (unpacked_message.message, unpacked_message.messNCB,
                                            unpacked_message.index, clServerPort,
                                            unpacked_message.data, unpacked_message.size));
                                break;
                            }
                            if (actConnection.connection->ncb_bufferLength_8 !=
                            ...
                            memcpy(actConnection.connection->ncb_buffer_4.p, unpacked_message.data,
                                   unpacked_message.size);
 ...
 for (connectionTime actConnection:handleConnections) {
                if (actClock > actConnection.startTime + actConnection.timeout) //after timeout
                {
                    ...
                    case 0x95:{//RECEIVE(opposite send)//fix it!!!!!!!!!!!!!!!!!!!
                            ...
                            debug_net_printf("processEnd: CONVERT FROM MESSAGE - timeout:%d:%d\n",
                                             ...
                                             10 /*strlen(tempstr.c_str()) */ );
                            debug_net_printf
                                ("processEnd: lastconnection set to NULL RECEIVE - timeout\n");
#endif                          //TEST_NETWORK_MESSAGES
                            break;
                        }
                    case 0xb0:{//ADD_NAME
    ...
    bool NetworkClass::HandleMessagesServerPre(std::string bufferStr,
    ...
    bool NetworkClass::HandleMessagesClientPre(std::string bufferStr,
                                               asio::ip::udp::endpoint sender) {
        message_info unpacked_message = Unpack_Message(bufferStr);
    ...
    void NetworkClass::HandleMessagesServer(std::string bufferStr, asio::ip::udp::endpoint sender) {
        ...
        if (unpacked_message.message == MESS_CLIENT_SERVER_NAME_ADDED) {
            ...
            if (serverAddname) {
                ...
                nullNCB.ncb_command_0 = 254;
                SendToClient(Pack_Message
                             (MESS_SERVER_SERVER_NAME_ADDED, nullNCB, unpacked_message.index, -10),
                             sender.address().to_string(), unpacked_message.port);
#ifdef TEST_NETWORK_MESSAGES
            ...
            if ((!GetNameNetwork(unpacked_message.data).compare(""))) {
                ...
                nullNCB.ncb_command_0 = 254;
                SendToClient(Pack_Message
                             (MESS_SERVER_TESTADDNAME_OK, nullNCB, unpacked_message.index, -10),
                             sender.address().to_string(), unpacked_message.port);
#ifdef TEST_NETWORK_MESSAGES
                ...
                nullNCB.ncb_command_0 = 254;
                SendToClient(Pack_Message
                             (MESS_SERVER_TESTADDNAME_REJECT, nullNCB, unpacked_message.index, -10),
                             sender.address().to_string(), unpacked_message.port);
#ifdef TEST_NETWORK_MESSAGES
            ...
            debug_net_printf("Server: MESSAGE_LISTEN:%s ; %s %s\n",
                             ((shadow_myNCB *) unpacked_message.data)->ncb_callName_10,
            ...
            if (AddListenName2(&unpacked_message.messNCB)) {
                SendToClient(Pack_Message
                             (MESS_SERVER_CALL_ACCEPT, unpacked_message.messNCB,
                ...
                debug_net_printf("Server: NETI_LISTEN_CONNECTED:%s %s %s\n",
                                 unpacked_message.messNCB.ncb_callName_10,
                                 unpacked_message.messNCB.ncb_name_26,
                                 sender.address().to_string().c_str());
#endif                          //TEST_NETWORK_MESSAGES
                ...
                SendToClient(Pack_Message
                             (MESS_SERVER_CALL_REJECT, unpacked_message.messNCB,
                              unpacked_message.index, -10), sender.address().to_string(),
                             unpacked_message.port);
                SendToClient(Pack_Message
        ...
        if (unpacked_message.message == MESS_SERVER_SERVER_NAME_ADDED) {
            ...
            if (ptrConn)
                debug_net_printf("CLIENT MESSAGE_CALL_ACCEPT: %p|%s|%s|\n", ptrConn->connection,
                                 ptrConn->connection->ncb_callName_10,
                                 ptrConn->connection->ncb_name_26);
            else
    ...
    void NetworkClass::Receiver2() {
        ...
        while (HandleReceiver2On) {
            ...
            message_info unpacked_message = Unpack_Message(bufferStr);
            if ((rand() % 3 == 0) || (unpacked_message.message == MESS_CLIENT_MESSAGE_LISTEN)
        ...
        while (HandleReceiverOn) {
            if (savedMessages.size() > 0) {
                ...
                debug_net_printf("Receive message: %s ip: %s port: %d\n", bufferStr.c_str(),
                                 sender.address().to_string().c_str(), clPort);
#endif                          //TEST_NETWORK_MESSAGES
    ...
    void NetworkClass::DeleteNetwork(myNCB * connection, int32_t index) {
        SendToServer(Pack_Message
                     (MESS_CLIENT_DELETE, myNCBtoShadow(*connection), index, clPort,
                      connection->ncb_name_26, sizeof(connection->ncb_name_26)));
    ...
    void NetworkClass::SendNetwork(myNCB * connection, int32_t index) {
        ...
        SendToServer(Pack_Message
                     (MESS_CLIENT_SEND, myNCBtoShadow(*connection), index, clPort,
                      (char *)connection->ncb_buffer_4.p, connection->ncb_bufferLength_8));
#ifdef TEST_NETWORK_MESSAGES
...
void CleanMessages(myNCB locNCB)
{
    ...
    for (auto it = recMessages.begin(); it != recMessages.end();) {
        ...
        if ((memcpy
             (unpackedMess.messNCB.ncb_name_26, locNCB.ncb_name_26, sizeof(locNCB.ncb_name_26)))
            ||
            (memcpy
             (unpackedMess.messNCB.ncb_callName_10, locNCB.ncb_name_26,
              sizeof(locNCB.ncb_name_26)))) {
...
{
#ifdef TEST_NETWORK_MESSAGES
    debug_net_printf("NetworkGetState: %d %p %d %s\n", 0, connections[0], connections[0]->ncb_lsn_2,
                     (!connections[0]->ncb_cmd_cplt_49) ? "true" : "false");
    debug_net_printf("NetworkGetState: %d %p %d %s\n", 1, connections[1], connections[1]->ncb_lsn_2,
                     (!connections[1]->ncb_cmd_cplt_49) ? "true" : "false");
    debug_net_printf("NetworkGetState: %d %p %d %s\n", 2, connections[2], connections[2]->ncb_lsn_2,
                     (!connections[2]->ncb_cmd_cplt_49) ? "true" : "false");
#endif                          //TEST_NETWORK_MESSAGES
}
