...
void InitializeLogging(spdlog::level::level_enum level)
{
    try {
        if (Logger == nullptr) {
            ...
            auto max_files = 3;
            auto file_sink =
                std::make_shared < spdlog::sinks::rotating_file_sink_mt > ("log.txt", max_size,
                                                                           max_files);
...
space_info space(char *path, int *ec)
{
    ...
    space_info info;
    if (!error(::BOOST_STATVFS(path, &vfs) ? BOOST_ERRNO : 0, p, ec, "boost::filesystem::space")) {
        info.capacity = static_cast < boost::uintmax_t > (vfs.f_blocks) * BOOST_STATVFS_F_FRSIZE;
        info.free = static_cast < boost::uintmax_t > (vfs.f_bfree) * BOOST_STATVFS_F_FRSIZE;
        info.available = static_cast < boost::uintmax_t > (vfs.f_bavail) * BOOST_STATVFS_F_FRSIZE;
    }
    ...
    if (GetDiskFreeSpaceExW(lpcwpath, &avail, &total, &free) != 0) {
        info.capacity = ((total.HighPart) << 32)
            + total.LowPart;
        info.free = ((free.HighPart) << 32)
            + free.LowPart;
        info.available = ((avail.HighPart) << 32)
            + avail.LowPart;
    }
...
unsigned __int64 dos_getdiskfree(__int16 a1, __int16 a2, uint8_t a, short *b)
{
    ...
    if (ec)
        if (myspaceinfo.free > wanted_size)
            return 0;
        else
            return 1;
    else
...
std::string getExistingDataPath(std::filesystem::path path)
{
    ...
    if (env_xdg_data_home_dir)
        xdg_data_home_dir = env_xdg_data_home_dir;
