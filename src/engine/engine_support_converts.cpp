...
void convert_struct_to_array_0x1AC_428(type_str_0x1AC_428 *input, uint8_t *output)
{
    //int16_t word_0;
...
void convert_struct_to_array_164_pos(axis_2d *input, uint8_t *output)
{
    //int16_t x;
...
void convert_struct_to_array_axis_2d(axis_2du *input, uint8_t *output)
{
    //uint16_t x;
    memcpy(output + 0x0, &input->x, 2);
    //uint16_t y;
...
void convert_struct_to_array_axis_3d(axis_3d *input, uint8_t *output)
{
    //uint16_t x;
    memcpy(output + 0x0, &input->x, 2);
    //uint16_t y;
...
void convert_struct_to_array_0x2BDE(type_str_0x2BDE *input, uint8_t *output)
{
    ...
    for (int i = 0; i < 0x21; i++)
        convert_struct_to_array_0x1d1_2BDE_11695(&input->struct_0x1d1_2BDE_11695[i],
                                                 output + 0x1d1 + i * 14);
...
void convert_struct_to_array_0x6E3E(type_str_0x6E3E *input, uint8_t *output)
{                               //10 length
...
void convert_struct_to_array_0x30311(type_entity_0x30311 *input, uint8_t *output)
{                               //length 20
    //uint16_t str_0x30311_type;//type_str_0x30311//type_str_0x30325//1091
    memcpy(output + 0, &input->type_0x30311, 2);
    //int16_t str_0x30311_subtype;//type_str_0x30313//type_str_0x30327//1091
    memcpy(output + 2, &input->subtype_0x30311, 2);
...
void convert_struct_to_array_0x360D2(type_str_0x360D2 *input, uint8_t *output)
{                               //length 110 //word_0x360D2
    //uint8_t stuba[3];//0
    memset(output + 0, 0, 2);
...
void convert_struct_to_array_0x36442(type_str_0x36442 *input, uint8_t *output)
{
    //int8_t byte_0;
    memcpy(output + 0, &input->index_0, 1);
    //int16_t word_1;//25973//word_0
    memcpy(output + 1, &input->stage_1, 2);
...
void convert_struct_to_array_0x3647Ac(type_str_0x3647Ac *input, uint8_t *output)
{                               //length 8
    //int8_t str_0x3647A_byte_0;//array_0x3647a
    memcpy(output + 0, &input->index_0x3647A_0, 1);
    //int8_t str_0x3647A_byte_1;//array_0x3647b
    memcpy(output + 1, &input->stage_0x3647A_1, 1);
...
void convert_struct_to_array_2FECE(type_str_2FECE *input, uint8_t *output)
{                               //length 6604
    ...
    //uint8_t stuba[1];//5
    memset(output + 5, 0, 1);
    //uint8_t byte_0x2FED4;//x_D41A0_BYTEARRAY_0[196308]//6
    memcpy(output + 6, &input->MapType, 1);
    //int16_t word_0x2FED5;//7
    memcpy(output + 7, &input->word_0x2FED5, 2);
    //int16_t word_0x2FED7;//9
    memcpy(output + 9, &input->word_0x2FED7, 2);
    //int8_t array_0x2FED9[8];//17
    memcpy(output + 17, input->player_0x2FED9, 8);
    //uint8_t stubb[4];//21
    memset(output + 0x21, 0, 4);
    //uint16_t word_0x2FEE5;//2FEE5//23
    memcpy(output + 23, &input->seed_0x2FEE5, 2);
    //uint8_t stubb2[2];//25
    memset(output + 0x20, 0, 2);
    //uint16_t word_0x2FEE9;//2FEE9//27 //first seed position
    memcpy(output + 5, &input->offset_0x2FEE9, 2);
    //uint8_t stubc[2];//29
    memset(output + 0x20, 0, 2);
    //uint16_t word_0x2FEED;//2FEED//31 //first seed height
    memcpy(output + 31, &input->raise_0x2FEED, 2);
    //uint8_t stubc2[2];//33
    memset(output + 0x20, 0, 2);
    //uint16_t word_0x2FEF1;//2FEF1//35
    memcpy(output + 35, &input->gnarl_0x2FEF1, 2);
    //uint8_t stubc3[2];//37
    memset(output + 0x20, 0, 2);
    //uint32_t word_0x2FEF5;//2FEF5//39
    memcpy(output + 37, &input->river_0x2FEF5, 4);
    //uint16_t word_0x2FEF9;//2FEF9//43
    memcpy(output + 43, &input->lriver_0x2FEF9, 2);
    //uint8_t stube[2];//45
    memset(output + 45, 0, 2);
    //uint16_t word_0x2FEFD;//2FEFD//47
    memcpy(output + 47, &input->source_0x2FEFD, 2);
    //uint8_t stubf[2];//49
    memset(output + 49, 0, 2);
    //uint16_t word_0x2FF01;//2FF01//51
    memcpy(output + 51, &input->snLin_0x2FF01, 2);
    //uint8_t stubg[2];//53
    memset(output + 0x20, 0, 2);
    //uint16_t word_0x2FF05;//2FF05//55
    memcpy(output + 55, &input->snFlt_0x2FF05, 2);
    //uint8_t stubh[2];//57
    memset(output + 57, 0, 2);
    //uint16_t word_0x2FF09;//2FF09//59
    memcpy(output + 59, &input->bhLin_0x2FF09, 2);
    //uint8_t stubi[2];//61
    memset(output + 61, 0, 2);
    //uint16_t word_0x2FF0D;//2FF0D//63
    memcpy(output + 63, &input->bhFlt_0x2FF0D, 2);
    //uint8_t stubj[2];//65
    memset(output + 65, 0, 2);
    //uint16_t word_0x2FF11;//2FF11//67
    memcpy(output + 5, &input->rkSte_0x2FF11, 2);
    //uint8_t stubk[1022];//69
    memset(output + 69, 0, 1022);
    //type_str_0x30311 array_0x30311[1200];//end(next entity) - 0x360d1//1091
    ...
    //type_str_0x360D2 next_0x360D2[8];//length 110//25092
    for (int i = 0; i < 0x8; i++)
    ...
    //type_str_0x3647Ac array_0x3647A[11];//8x11//26028
    for (int i = 0; i < 0xb; i++)
        convert_struct_to_array_0x3647Ac(&input->StageVars_0x3647A[i], output + 26028 + i * 8);
}                               //compress level 

void convert_struct_to_array_0x364D2(type_str_0x364D2 *input, uint8_t *output)
{                               //length 108
...
void convert_struct_to_array_3654C(type_str_3654C *input, uint8_t *output)
{                               //size 10 count 8
...
void convert_struct_to_array_0x3664C(type_str_0x3664C *input, uint8_t *output)
{                               //length 39
    //int8_t byte_0;
...
void convert_struct_to_array_D41A0_0(type_D41A0_BYTESTR_0 *input, uint8_t *output)
{
    ...
    memcpy(output + 0xc, &input->LevelIndex_0xc, 2);
    //int16_t word_0xe;
    ...
    //type_str_0x2BDE array_0x2BDE[8]; //0x84c*0x8 ??
    for (int i = 0; i < 0x8; i++)
    ...
    convert_struct_to_array_2FECE(&input->terrain_2FECE, output + 0x2FECE);
    ...
    //type_str_3654C struct_0x3654C[8];//size 10 count 8
    for (int i = 0; i < 0x8; i++)
    ...
    //type_str_0x3647Ac array_0x365F4[11];//8x11    //set 0x58 // end 0x3664c
    for (int i = 0; i < 0x8; i++)
