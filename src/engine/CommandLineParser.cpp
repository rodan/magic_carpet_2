...
void CommandLineParser::InterpretParams()
{
    ...
    "--mode_release_game",
            ...
            "--mode_debug_afterload", "--mode_debug_onstart", "--mode_test_network",};
    ...
    for (auto p = params.cbegin(); p != params.cend(); ++p) {
        ...
        if (param == "--mode_release_game") {   //this is standard setting
            ...
            m_text_output_to_console = true;
            m_show_debug_messages1 = true;
        } else if (param == "--no_alternative_gamespeed_control")
            m_alternative_gamespeed_control = false;
        else if (param == "--no_analyze_entity")
            m_analyze_entity = false;
        else if (param == "--auto_change_res")
            m_auto_change_res = true;
        else if (param == "--copy_skip_config")
            m_copy_skip_config = true;
        else if (param == "--debug_sequences")
            m_debug_sequences = true;
        else if (param == "--debug_sequences2")
            m_debug_sequences2 = true;
        else if (param == "--detect_dword_a")
            m_detect_dword_a = true;
        else if (param == "--disable_graphics_enhance")
            m_disable_graphics_enhance = true;
        else if (param == "--fix_flyasistant")
            m_fix_flyasistant = true;
        else if (param == "--fix_mouse")
            m_fix_mouse = true;
        else if (param == "--interval_save")
            m_interval_save = true;
        else if (param == "--load_edited_level")
            m_load_edited_level = true;
        else if (param == "--mouse_off")
            m_mouse_off = true;
        else if (param == "--mouse_off2")
            m_mouse_off2 = true;
        else if (param == "--move_player")
            m_move_player = true;
        else if (param == "--no_show_new_procedures")
            m_no_show_new_procedures = true;
        else if (param == "--off_pause_5")
            m_off_pause_5 = true;
        else if (param == "--right_button")
            m_right_button = true;
        else if (param == "--test_regression")
            m_test_regression = true;
        else if (param == "--test_renderers")
            m_test_renderers = true;
        else if (param == "--debugafterload")
            m_debugafterload = true;
        else if (param == "--graphics_debug")
            m_graphics_debug = true;
        else if (param == "--hide_graphics")
            m_hide_graphics = true;
        else if (param == "--rotate_player")
            m_rotate_player = true;
        else if (param == "--set_objective")
            m_set_objective = true;
        else if (param == "--set_level")
            m_set_level = true;
        else if (param == "--test_network_chng1")
            m_test_network_chng1 = true;
        else if (param == "--show_debug_messages1")
            m_show_debug_messages1 = true;
        else if (param == "--show_debug_show_periphery")
            m_show_debug_periphery = true;
        else if (param == "--text_output_to_console")
            m_text_output_to_console = true;
