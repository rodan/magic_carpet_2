...
typedef MSS_STRUCT              // Standard MSS 3.X VDI driver header
{
    ...
    /*12 */ uint16_t VDI_HDR_var12;
                                                /*14 */ uint16_t num_IO_configurations_14;
                                                //03
                                                /*16 */ uint32_t environment_string_16;
                                                //2c380506 
    //20
                                /*22 */ IO_PARMS IO;
                                //22
    /*20 */ uint16_t scratch0;
                                        /*46 */ int16_t VDI_HDR_var46;
                                        //busy
                                        /*48 */ uint16_t VDI_HDR_var48;
                                        //driver_num
    /*50 */ uint16_t VDI_HDR_var50;
    /*52 */ uint16_t VDI_HDR_var52_this_ISR;
    /*54 */ uint32_t VDI_HDR_var54_prev_ISR;
                                        /*58 */ int8_t scratch[128];
...
{
                                /*0 */ REALFAR seg_0;
                                //remove this! segment must be zero!
/*4*/ uint32_t sel_1;
/*8*/ uint8_t *buf_2;
/*12*/ uint32_t size_3;
/*16*/ VDI_HDR *VHDR_4;
...
{
                                                /*53 0 */ int32_t program[NUM_CHANS];
                                                // Program Change
                                                /*69 16 */ int32_t pitch_l[NUM_CHANS];
                                                // Pitch Bend LSB
                                                /*85 32 */ int32_t pitch_h[NUM_CHANS];
                                                // Pitch Bend MSB

                                                /*101 48 */ int32_t c_lock[NUM_CHANS];
                                                // Channel Lock
                                                /*117 64 */ int32_t c_prot[NUM_CHANS];
                                                // Channel Lock Protection
                                                /*133 80 */ int32_t c_mute[NUM_CHANS];
                                                // Channel Mute
                                                /*149 96 */ int32_t c_v_prot[NUM_CHANS];
                                                // Voice Protection
                                                /*165 112 */ int32_t bank[NUM_CHANS];
                                                // Patch Bank Select
                                                /*181 128 */ int32_t indirect[NUM_CHANS];
                                                // ICA indirect controller value
                                                /*197 144 */ int32_t callback[NUM_CHANS];
                                                // Callback Trigger

                                                /*213 160 */ int32_t mod[NUM_CHANS];
                                                // Modulation
                                                /*229 176 */ int32_t vol[NUM_CHANS];
                                                // Volume
                                                /*245 192 */ int32_t pan[NUM_CHANS];
                                                // Panpot
                                                /*261 208 */ int32_t exp[NUM_CHANS];
                                                // Expression
                                                /*277 224 */ int32_t sus[NUM_CHANS];
                                                // Sustain
                                                /*393 240 */ int32_t reverb[NUM_CHANS];
                                                // Reverb
                                                /*309 256 */ int32_t chorus[NUM_CHANS];
                                                // Chorus

                                                /*325 272 */ int32_t bend_range[NUM_CHANS];
                                                // Bender Range (data MSB, RPN 0 assumed)
    //341 288
} CTRL_LOG;

typedef struct                  // XMIDI sequence state table
{
                                                /*0 */ struct _MDI_DRIVER *driver_0;
                                                // Driver for playback //0

                                /*1 */ uint32_t status_1;
                                // SEQ_ flags

                                /*2 */ void *TIMB_2;
                                // XMIDI IFF chunk pointers
    /*3 */ uint8_t *RBRN_3;
    /*4 */ void *EVNT_4;

                                /*5 */ uint8_t *EVNT_ptr_5;
                                // Current event pointer

                                /*6 */ uint8_t *ICA_6;
                                // Indirect Controller Array

                                        /*7 */ xCALLBACK prefix_callback_7;
                                        // XMIDI Callback Prefix handler
                                                /*8 */ xCALLBACK trigger_callback_8_32;
                                                // XMIDI Callback Trigger handler
                                        /*9 */ xCALLBACK beat_callback_9;
                                        // XMIDI beat/bar change handler
                                /*10 */ xCALLBACK EOS_10;
                                // End-of-sequence callback function

                                        /*11 */ int32_t loop_count_11;
                                        // 0=one-shot, -1=indefinite, ...

                                        /*12 */ int32_t interval_count_12;
                                        // # of intervals until next event
                                        /*13 */ int32_t interval_num_13;
                                        // # of intervals since start

                                /*14 */ int32_t volume_14;
                                // Sequence volume 0-127
                                        /*15 */ int32_t volume_target_15;
                                        // Target sequence volume 0-127
                                        /*16 */ int32_t volume_accum_16;
                                        // Accumulated volume period
                                        /*17 */ int32_t volume_period_17;
                                        // Period for volume stepping

                                        /*18 */ int32_t tempo_percent_18;
                                        // Relative tempo percentage 0-100
                                        /*19 */ int32_t tempo_target_19;
                                        // Target tempo 0-100
                                        /*20 */ int32_t tempo_accum_20;
                                        // Accumulated tempo period
                                        /*21 */ int32_t tempo_period_21;
                                        // Period for tempo stepping
                                        /*22 */ int32_t tempo_error_22;
                                        // Error counter for tempo DDA

                                        /*23 */ int32_t beat_count_23;
                                        // Sequence playback position
    /*24 */ int32_t measure_count_24;

                                        /*25 */ int32_t time_numerator_25;
                                        // Sequence timing data
    /*26 */ int32_t time_fraction_26;
    /*27 */ int32_t beat_fraction_27;
    /*28 */ int32_t time_per_beat_28;

                                                /*29 */ uint8_t *FOR_ptrs[FOR_NEST];
                                                // Loop stack
    /*33 */ int32_t FOR_loop_count_33[FOR_NEST];

                                                /*37 */ int32_t chan_map_37[NUM_CHANS];
                                                // Physical channel map for sequence

                                /*53 */ CTRL_LOG shadow_53;
                                // Controller values for sequence

                                        /*341 */ int32_t note_count;
                                        // # of notes "on"

                                                /*342 */ int32_t note_chan[MAX_NOTES];
                                                // Channel for queued note (-1=free)
                                                /*374 */ int32_t note_num[MAX_NOTES];
                                                // Note # for queued note
                                                /*406 */ int32_t note_time[MAX_NOTES];
                                                // Remaining duration in intervals

                                        /*438 */ int32_t user_data[8];
                                        // Miscellaneous user data
                                        /*446 */ int32_t system_data[8];
...
typedef struct _MDI_DRIVER      // Handle to XMIDI driver
{
    ...
    uint32_t n_sequences_7;
                                        ...
                                        /*8 */ int32_t lock[NUM_CHANS];
                                        // 1 if locked, 2 if protected, else 0
                                                /*24 */ HSEQUENCE locker[NUM_CHANS];
                                                // HSEQUENCE which locked channel
                                        /*40 */ HSEQUENCE owner[NUM_CHANS];
                                        // HSEQUENCE which owned locked channel
                                        /*56 */ HSEQUENCE user[NUM_CHANS];
                                        // Last sequence to use channel
                                        /*72 */ int32_t state[NUM_CHANS];
                                        // Lock state prior to being locked

                                        /*88 */ int32_t notes[NUM_CHANS];
                                        // # of active notes in channel

                                        /*104 */ xCALLBACK event_trap;
                                        // MIDI event trap callback function
                                        /*105 */ xCALLBACK timbre_trap;
                                        // MIDI timbre request callback function

                                        /*106 */ int32_t message_count;
                                        // MIDI message count
                                /*107 */ int32_t offset;
                                // MIDI buffer offset

                                        /*108 */ int32_t master_volume;
                                        // Master XMIDI note volume 0-127

                                        /*109 */ int32_t system_data[8];
                                        // Miscellaneous system data
