...
unsigned int sub_71090(unsigned int a1) //252090
{
    ...
    do {
        ...
        if (str_DWORD_F66F0x[v4] && !m_LevelSpriteList_F5340[v4])
            v31 = 0;
        while ((unsigned __int16)v3 < 0x1F8u && str_TMAPS00TAB_BEGIN_BUFFER[v3 + 1].word_8 == v4)
            ++v3;
        ++v3;
    ...
    do {
        ...
        while ((unsigned __int16)v5 < 0x1F8u && str_TMAPS00TAB_BEGIN_BUFFER[v5 + 1].word_8 == v6)
            ++v5;
        ++v5;
...
char sub_70E10(unsigned __int16 a1)     //251e10
{
    ...
    if (!str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8)
        return 0;
    for (i = str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8;
         i < 0x1F8u
         && str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8 == str_TMAPS00TAB_BEGIN_BUFFER[i].word_8; i++) {
...
char sub_70D20(unsigned __int16 a1)     //251d20
{
    ...
    if (!str_DWORD_F66F0x[str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8])
        return 0;
    for (i = str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8;
         i < 504 && str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8 == str_TMAPS00TAB_BEGIN_BUFFER[i].word_8;
         i++) {
        v4x = str_DWORD_F66F0x[i];
        if (v4x) {
            if ((*v4x)->word_0 & 1) {
...
void InitTmaps(unsigned __int16 a1)     //251f50
{
    ...
    for (i = str_TMAPS00TAB_BEGIN_BUFFER[a1].word_8; i < 504; i++) {
        ...
        if (!str_DWORD_F66F0x[i]) {
            index3x =
                LoadTMapMetadata_71E70(x_DWORD_E9C28_str,
                                       (unsigned __int16)(4 *
                                                          ((unsigned
                                                            int)(str_TMAPS00TAB_BEGIN_BUFFER[i].
                                                                 dword_0 + 13) >> 2)), i);
            //v4 = index3;
            if (index3x) {
                ...
                if (index4 != -1) {
                    if (bigSprites) {
                        ...
                        if ((oldwidth > 0) && (oldwidth < 512) && (oldheight > 0)
                            && (oldheight < 512)) {
                            int actnumber = 0;
                            ...
                            for (mm = 0; mm < max_sprites_frames; mm++) {
                                ...
                                BIG_SPRITES_BUFFERx[i].frames[mm] =
                                    (uint8_t *) malloc(oldwidth * 4 * oldheight * 4);
                                //fread(BIG_SPRITES_BUFFERx[i].frames[mm], oldwidth * 4 * oldheight * 4, 1, fptr_outdata);
                                //myclose(fptr_outdata);
                                ReadGraphicsfile(filebuffer, BIG_SPRITES_BUFFERx[i].frames[mm],
                                                 oldwidth * 4 * oldheight * 4);
                            }

                            BIG_SPRITES_BUFFERx[i].actdatax =
                                (type_particle_str *) malloc(oldwidth * 4 * oldheight * 4 + 6 + 2);
                            memcpy(BIG_SPRITES_BUFFERx[i].actdatax->textureBuffer,
                                   BIG_SPRITES_BUFFERx[i].frames[0], oldwidth * 4 * oldheight * 4);
                            ...
                            *(uint16_t *) & BIG_SPRITES_BUFFERx[i].actdatax->
                                textureBuffer[oldwidth * 4 * oldheight * 4] = mm;
...
subtype_x_DWORD_E9C28_str *LoadTMapMetadata_71E70(type_x_DWORD_E9C28_str *a1y, unsigned int a2, __int16 a3)     //252e70
{
    ...
    if (a2 < a1y->dword_4) {
        ...
        if (idx > -1) {
            ...
            a1y->str_8_data[idx].partstr_0 =
                (type_particle_str *) (a1y->dword_16x + (a1y->dword_0 - a1y->dword_4));
...
int sub_70C60_decompress_tmap(uint16_t texture_index, uint8_t *texture_buffer)  //251c60
{
    ...
    int v3 =
        str_TMAPS00TAB_BEGIN_BUFFER[texture_index + 1].dword_4 -
        str_TMAPS00TAB_BEGIN_BUFFER[texture_index].dword_4;
...
void WriteTextureMapToBmp(uint16_t texture_index, type_particle_str *ptextureMap, MapType_t mapType)
{
    ...
    BitmapIO::WriteRGBAImageBufferAsImageBMP(path.c_str(), ptextureMap->width, ptextureMap->height,
                                             m_pColorPalette,
                                             (uint8_t *) ptextureMap->textureBuffer);
}
