...
type_E37A0_sound_buffer2 *soundIndex_E37A0 = 0; // pointer to the sound bank ToC inside in-memory copy of sounds.dat
//uint8_t* x_DWORD_E37A0_sound_buffer2 = 0; // weak
//replace str2_E37A4_sound_buffer3 and x_DWORD_E380C with equvivalent index_E37A4_MaxSound and index_E380C_MaxMusic
int numOfLoadedSounds_E37A4 = 0;
//int8_t* str2_E37A4_sound_buffer3 = 0; // weak
uint8_t *soundBuffer1_E37A8 = nullptr;  // weak   sound bank pointer to actual wav chunks inside in-memory copy of sounds.dat
...
uint32_t x_DWORD_180B4C_end_sound_buffer3_endindex;     // number of voices
...
int x_DWORD_A9390[128] = {
    ...
    0x00005266, 0x0000574C, 0x00005C80, 0x00006200
};                              // weak
...
void InitSound_8D290()          //26e290
{
    ...
    if (mdSound) {
        sscanf((char *const)mdSound, "%s %x %d %d %d", mdSoundPar1, &ioParms.IO, &ioParms.IRQ,
               &ioParms.DMA_8_bit, &ioParms.DMA_16_bit);
...
void InitMusic_8D970()          //26e970
{
    ...
    if (mdMusic) {
        sscanf((char *const)mdMusic, "%s %x %d %d %d", mdMusicPar1, &ioParms.IO, &ioParms.IRQ,
               &ioParms.DMA_8_bit, &ioParms.DMA_16_bit);
    ...
    if (sub_969A0_AIL_MDI_driver_type(hMdiMusicDriver) == 1
        || sub_969A0_AIL_MDI_driver_type(hMdiMusicDriver) == 2) {
        musicDriverType_180C84 = 'f';
        isDriver = true;
    }
    if (isDriver) {
...
void StopMusic_8E020()          //26f020
{
    if (musicAble_E37FC && musicActive_E37FD && songCurrentlyPlaying_E3802) {
        if (x_BYTE_E3818) ;     // sub_92DC0_AIL_release_timer_handle(x_DWORD_180C80);
...
{
    if (musicAble_E37FC && musicActive_E37FD && track <= m_iNumberOfTracks
        ...
        AilInitSequence_95C00(m_hSequence, musicHeader_E3808->str_8.track_10[track].xmiData_0, 0,
                              track);
...
void sub_917D0()
{
    ...
    dbgfprintf(ailDebufFile_181BF0,
               "-----------------------------------------------------------------");
    dbgfprintf(ailDebufFile_181BF0,
               "Audio Interface Library application usage script generated by AIL", "3.03");
...
{
    // looks like channels is always 0, as well as byteSampleType
    ailIndent_181C04++;
    if (x_DWORD_181BF4 && (ailIndent_181C04 == 1 || x_DWORD_181BF8) && !GetE3FFE_A16A2()
        && DebugSoundTimer_916F0())
...
//----- (00095C00) --------------------------------------------------------
int32_t AilInitSequence_95C00(HSEQUENCE hSequence, uint8_t *start, int32_t sequence_num,
                              uint32_t track)
{
    ailIndent_181C04++;
    if (x_DWORD_181BF4 && (ailIndent_181C04 == 1 || x_DWORD_181BF8) && !GetE3FFE_A16A2()
...
{
    if (musicAble_E37FC && musicActive_E37FD && songCurrentlyPlaying_E3802 && volume <= 0x7Fu
        && AilSequenceStatus_96170(m_hSequence) != 2)
...
void LoadSoundDataFromBuffer_844A0(uint16_t count)      //2654a0
{
    ...
    if (soundIndex_E37A0 && soundBuffer1_E37A8) {
        for (index = 0; index < count; index++) {
            ...
            soundIndex_E37A0->str_8.wavs_10[index].wavData_0 =
                reinterpret_cast < uint32_t >
                (soundIndex_E37A0->str_8.wavs_10[index].wavData_0) + soundBuffer1_E37A8;
...
bool ReadAndDecompressSound(FILE *file, uint8_t soundIndex2)    //2654f0
{
    ...
    if (!soundActiveH_E2A14) {
        ...
        soundIndex_E37A0 =
            (type_E37A0_sound_buffer2 *) Malloc_83CD0(sizeof(type_E37A0_sound_buffer2));
    ...
    //64xfix
    shadow_str_E37A0_sound_buffer2 =
        (shadow_type_E37A0_sound_buffer2 *) calloc(1, sizeof(shadow_type_E37A0_sound_buffer2));
    ...
    numOfLoadedSounds_E37A4 =
        (soundBank2[soundIndex2].sizeBytes_8) / sizeof(shadow_sub2type_E37A0_sound_buffer2);
    DataFileIO::Seek(file, soundBank2[soundIndex2].dword_4, 0);
    DataFileIO::Read(file, soundBuffer1_E37A8, 8);
    if (soundBuffer1_E37A8[0] != 'R' || soundBuffer1_E37A8[1] != 'N'
        || soundBuffer1_E37A8[2] != 'C') {
        DataFileIO::Read(file, (soundBuffer1_E37A8 + 8), soundBank2[soundIndex2].dword_12 - 8);
    } else {
        DataFileIO::Read(file, (soundBuffer1_E37A8 + 8),
                         soundBuffer1_E37A8[7] +
                         ((soundBuffer1_E37A8[6] +
                           ((soundBuffer1_E37A8[5] + (soundBuffer1_E37A8[4] << 8)) << 8)) << 8) -
                         8);
    ...
    if (shadow_str_E37A0_sound_buffer2->byte_0 != 'R'
        || shadow_str_E37A0_sound_buffer2->byte_1 != 'N'
        || shadow_str_E37A0_sound_buffer2->byte_2 != 'C') {
        // go back to the start of the bank, since this block has no header
        DataFileIO::Seek(file, soundBank2[soundIndex2].dword_0, 0);
        DataFileIO::Read(file, (uint8_t *) & shadow_str_E37A0_sound_buffer2->str_8, soundBank2[soundIndex2].sizeBytes_8);       // populates wavs_10
    } else {
        DataFileIO::Read(file, (uint8_t *) & shadow_str_E37A0_sound_buffer2->str_8,
                         shadow_str_E37A0_sound_buffer2->byte_7 +
                         ((shadow_str_E37A0_sound_buffer2->byte_6 +
                           ((shadow_str_E37A0_sound_buffer2->byte_5 +
                             (shadow_str_E37A0_sound_buffer2->byte_4 << 8)) << 8)) << 8) - 8);
        DataFileIO::Decompress((uint8_t *) shadow_str_E37A0_sound_buffer2,
                               (uint8_t *) shadow_str_E37A0_sound_buffer2);
    }
    ...
    for (int i = 0; i < numOfLoadedSounds_E37A4; i++) {
        soundIndex_E37A0->str_8.wavs_10[i].wavData_0 =
            (uint8_t *) shadow_str_E37A0_sound_buffer2->str_8.wavs_10[i].wavData_0;
        soundIndex_E37A0->str_8.wavs_10[i].wavSize_8 =
            shadow_str_E37A0_sound_buffer2->str_8.wavs_10[i].wavSize_8;
        soundIndex_E37A0->str_8.wavs_10[i].word_12 =
            shadow_str_E37A0_sound_buffer2->str_8.wavs_10[i].word_12;
        memcpy(soundIndex_E37A0->str_8.wavs_10[i].filename_14,
               shadow_str_E37A0_sound_buffer2->str_8.wavs_10[i].filename_14, 18);

        //Logger->info("ReadAndDecompressSound  id {}  sz {}  offset {}  {}", i, soundIndex_E37A0->str_8.wavs_10[i].wavSize_8, soundBank2[soundIndex2].dword_0 + 32*i, soundIndex_E37A0->str_8.wavs_10[i].filename_14);
    }
    ...
    for (int i = 0; i < numOfLoadedSounds_E37A4; i++) {
        if (soundIndex_E37A0->str_8.wavs_10[i].wavSize_8 > 1000) {
            data = soundIndex_E37A0->str_8.wavs_10[i].wavData_0;
            snprintf(fname, 64, "/tmp/%02d_%s", i, soundIndex_E37A0->str_8.wavs_10[i].filename_14);
            alsound_save_chunk(data, soundIndex_E37A0->str_8.wavs_10[i].wavSize_8, fname);
        }
    }
#endif
...
AIL_DRIVER *AilApiInstallDriver_9E720(uint8_t *driver_image, int32_t n_bytes)   //27f720
{
    ...
    if (ailDriver) {
        ...
        if (AllocMem_9D4D0
            ((n_bytes + 15) >> 4, &ailDriver->buf_2, &ailDriver->seg_0, &ailDriver->sel_1)) {
...
{
    if (ailDriver->server_8 != -1) ;    // sub_92DC0_AIL_release_timer_handle(a1->server_8);
...
int sub_9EE70()
{
    if (!sampleFinished_E3E45 && AilSampleUserData_95480(mainSample_181E10, x_DWORD_E3E40)
        && (unsigned int)x_DWORD_E3E40 < 8) {
        AilInitSample_93830(mainSample_181E10);
        AilRegisterEosCallback_95140(mainSample_181E10, sub_9EE70);
        AilSetSampleFile_938C0(mainSample_181E10,
                               soundIndex_E37A0->str_8.
                               wavs_10[AilSampleUserData_95480(mainSample_181E10, x_DWORD_E3E40)].
                               wavData_0, 1);
        x_DWORD_E3E40++;
...
{
    if (musicAble_E37FC && !_stricmp(textBuffer_180BE0, "SBAWE32.MDI")
        && !_stricmp(&musicDriverType_180C84, "w")) {
            ...
            if ((x_DWORD) v1 || x_WORD_181E30) {
                ...
                if (v9 != -1) {
                    ...
                    if (sub_9F2E0(hMdiMusicDriver, v5x.par1, v5x.par2, 2u).AX == -1     //fix
                        && !dos_read(file, x_DWORD_181E2C, (unsigned __int16)x_WORD_181E30, 512,
                                     &v8)) {
                        ...
                        if ((x_DWORD) v2 || v5x.par2) {
                            ...
                            for (i = 0; (signed __int16)__readgsx_WORD(v5x.par1) > i; i++) {
                                if (dos_read
                                    (file, x_DWORD_181E2C, (unsigned __int16)x_WORD_181E30, 512,
                                     &v8)
                            ...
                            if ((x_DWORD) v3 || x_WORD_181E2A) {
                                ...
                                if (!dos_read(file,
                                              ...
                                              __readgsx_DWORD(v5x.par1 + 10), &v8)
...
void InitSampleVolume_A2110(HSAMPLE S)  //283110
{
    ...
    if (volumeA > 127)
        volumeA = 127;
    if (S->format_13 != 1 && S->format_13 != 3) {       //set volume table
        if (volumeA)
            volumeA++;
        if (S->sam_var[6] != 2
            && S->sam_var[6] != 3
        ...
        if (digDriver->drvr_0->type_5) {
            ...
            if (ioInitited) {
                ...
                if (sub_A2650(digDriver)) {
                    ...
                    digDriver->samples_per_buffer_18 =
                        digDriver->half_buffer_size_4 / (digDriver->bytes_per_channel_16 *
                                                         digDriver->channels_per_sample_15);
                    digDriver->channels_per_buffer_17 =
                        digDriver->half_buffer_size_4 / digDriver->bytes_per_channel_16;
                    ...
                    if (digDriver->build_buffer_20) {
                        ...
                        digDriver->samples_23 =
                            (_SAMPLE *) calloc(digDriver->n_samples_24, sizeof(_SAMPLE));
                        if (digDriver->samples_23) {
                            ...
                            if (digDriver->timer_3 == -1) {
                                ...
                                FreeMem_9D560((uint8_t *) digDriver->DMA_buf_10);
                                FreeMem_9D490(digDriver->samples_23,
                                              sizeof(_SAMPLE) * digDriver->n_samples_24);
                            ...
                            qmemcpy(textBuffer_181C90,
                                    (void *)"Could not allocate SAMPLE structures\n", 38);
                            if (ailDriver->PM_ISR_7 != -1)
                                AilRestoreUSE16ISR_91E90(ailDriver->PM_ISR_7);
                            AilCallDriver_91F70(digDriver->drvr_0, 774, nullptr, nullptr);
                            digDriver->n_samples_24 = 0;
...
int AilApiInstallDigINI_A3720(HDIGDRIVER *dig, char *digPath)   //284720
{
    int result;
    AIL_INI ailIni;
    if (AilReadIniFile_92190(&ailIni, digPath)) {       //3550c8 - SB16.DIG  //355148 - 220
...
void InitHqsound(HSAMPLE S)
{
    ...
    if (comparesize > S->len_4_5[0]) {
        comparesize = S->len_4_5[0];
    }
    ...
    if (!same_mark) {
        ...
        if (S->start_44mhz != nullptr) {
            free(S->start_44mhz);
            S->start_44mhz = nullptr;   // double free crash if you remove this line
        }
...
{
    if (S) {
        //Logger->info("SetSampleAddress_A3A30 start {}, len {}", start, len); FIXME
        S->start_2_3[0] = start;
        S->start_2_3[1] = 0;
        S->len_4_5[0] = len;
        S->len_4_5[1] = 0;
...
void SubSetSampleFile_A47C0(uint8_t *pWaveData, HSAMPLE S)      //2857c0
{
    ...
    for (int i = _strnicmp((const char *)&pWaveData[12], "fmt ", 4); i;
         i = _strnicmp((const char *)&pWaveData[index], "fmt ", 4))
        index +=
            (*(int32_t *) & pWaveData[index + 4] & 1) + *(int32_t *) & pWaveData[index + 4] + 8;
    ...
    index = 12;
    for (int j = _strnicmp((const char *)&pWaveData[12], "data", 4); j;
         j = _strnicmp((const char *)&pWaveData[index], "data", 4))
        index +=
            (*(int32_t *) & pWaveData[index + 4] & 1) + *(int32_t *) & pWaveData[index + 4] + 8;
    ...
    if (a3 < 0x6F) {
        ...
        if (a3 <= 0x6C) {       //6c
            if (hSequence->prefix_callback_7)
                hSequence->shadow_53.indirect[indexCh] =
                    (*(int (**)(HSEQUENCE, int, signed int))(hSequence->prefix_callback_7))
                    (hSequence, indexCh, a4);
            goto LABEL_79;
        }
        if (a3 <= 0x6D) {       //6d
        ...
        if (a3 < 0x73)
            goto LABEL_79;
        if (a3 <= 0x73) {       //73
            *(x_DWORD *) (hSequence + 4 * indexCh + 724) =
                *(unsigned __int8 *)(a4 + *(x_DWORD *) (hSequence + 24));
 LABEL_79:                     //end
            if (mdiDriver->lock[chMap] != 1 || mdiDriver->locker[chMap] == hSequence) {
                ...
                if (indexChH != 144 || hSequence->shadow_53.c_mute[indexCh] < 64) {
                    if (!mdiDriver->event_trap
                        ||
                        ((HMDIDRIVER) (x_DWORD *)
                         ((int (*)(HMDIDRIVER, HSEQUENCE, int, unsigned int, signed int))mdiDriver->
                          event_trap) (mdiDriver, hSequence, chMap | indexChH, a3, (int)a4)) == 0) {
        ...
        *(x_DWORD *) (hSequence + 108) -= *(x_DWORD *) (hSequence + 104);
        if (hSequence->beat_callback_9)
            (*(int (**)(HMDIDRIVER, HSEQUENCE, x_DWORD, x_DWORD))(hSequence->beat_callback_9))
                (mdiDriver, hSequence, 0, 0);
    } else {
        if (a3 != 119)
            goto LABEL_79;
        if (hSequence->trigger_callback_8_32)
            (*(int (**)(HSEQUENCE, int, signed int))(hSequence->trigger_callback_8_32)) (hSequence,
                                                                                         indexCh,
                                                                                         a4);
    }
}
        ...
        if (hMdiDriver->drvr_0->type_5 == 1) {
            ...
            if (ioIsSet) {
                ...
                if (vdiOut.AX) {
                    ...
                    hMdiDriver->sequences_6 =
                        (HSEQUENCE) malloc(sizeof(SEQUENCE) * hMdiDriver->n_sequences_7);
                    if (hMdiDriver->sequences_6) {
                        ...
                        qmemcpy(textBuffer_181C90,
                                (void *)"Could not allocate SEQUENCE structures\n", 40);
...
//----- (000A7C20) --------------------------------------------------------
int32_t AilApiInitSequence_A7C20(HSEQUENCE hSequence, void * /*start */ , int32_t sequence_num,
                                 uint32_t track)
{
...
void AilAPISetSequenceVolume_A8360(HSEQUENCE hSequence, int32_t volume, int32_t milliseconds)   //289360
{
    if (hSequence) {
        ...
        if (hSequence->volume_14 == hSequence->volume_target_15) {
            ...
            if (milliseconds) {
                hSequence->volume_period_17 =
                    1000 * milliseconds / abs(hSequence->volume_14 - hSequence->volume_target_15);
                hSequence->volume_accum_16 = 0;
            } else {
...
int AilApiLockChannel_97F90_A8BF0(MDI_DRIVER *mdi)
{
    ...
    if (v5 == -1) {
        ...
        while (v9) {
            if (*(x_DWORD *) (v3 + 4) != 1) {
                for (int k = 0; k < 32; k++) {
                    if (*(x_DWORD *) (v3 + 4 * k + 1368) != -1
                        && *(x_DWORD *) (v3 + 4 * *(x_DWORD *) (v3 + 4 * k + 1368) + 148) == v5) {
                        sub_A5850(v3, *(unsigned int *)(v3 + 4 * k + 1368) | 0x80,
                                  *(x_DWORD *) (v3 + 4 * k + 1496), 0, 0);
...
bool DebugSoundTimer_916F0()
{
    ...
    if (ailIndent_181C04 == 1) {
        dbgfprintf(ailDebufFile_181BF0, "[%.02u:%.02u:%.02u.%.02u] ", x_DWORD_181BFC, timer,
                   timeptr, x_DWORD_181C10);
    } else {
        for (int i = 0; i < 14; i++)
            dbgfprintf(ailDebufFile_181BF0, " \0");
    }
...
void GetMusicSequenceCount()    //26fc90 // set index
{
    if (musicHeader_E3808 && musicData_E3810) {
        for (m_iNumberOfTracks = 0; m_iNumberOfTracks < countOfMusicTracks_E380C;
             m_iNumberOfTracks++) {
            uint8_t *zero_pointer = 0;
            musicHeader_E3808->str_8.track_10[m_iNumberOfTracks].xmiData_0 =
                &musicData_E3810[musicHeader_E3808->str_8.track_10[m_iNumberOfTracks].xmiData_0 -
                                 zero_pointer];
        }
    }
}
...
bool LoadMusicTrack(FILE *filehandle, uint8_t drivernumber)     //26fd00
{
    ...
    countOfMusicTracks_E380C =
        (headerx[drivernumber].sizeBytes_8) / sizeof(sub2type_E37A0_sound_buffer2);
    ...
    if (musicData_E3810[0] == 'R' && musicData_E3810[1] == 'N' && musicData_E3810[2] == 'C')    //RNC
    {
        ...
        DataFileIO::Read(filehandle, (uint8_t *) & musicData_E3810[8],
                         headerx[drivernumber].dword_12 - 8);
    }
    ...
    if (shadow_str_E3808_music_header->byte_0 == 'R' && shadow_str_E3808_music_header->byte_1 == 'N' && shadow_str_E3808_music_header->byte_2 == 'C')   //RNC
    {
        ...
        DataFileIO::Read(filehandle, (uint8_t *) (&shadow_str_E3808_music_header->str_8),
                         rncsize - 8);
        DataFileIO::Decompress((uint8_t *) shadow_str_E3808_music_header,
                               (uint8_t *) shadow_str_E3808_music_header);
    } else {
        DataFileIO::Read(filehandle, (uint8_t *) (&shadow_str_E3808_music_header->str_8),
                         headerx[drivernumber].sizeBytes_8 - 8);
    }
    ...
    for (int i = 0; i < 6; i++) {
        musicHeader_E3808->str_8.track_10[i].xmiData_0 =
            shadow_str_E3808_music_header->str_8.track_10[i].xmiData_0 + Zero_pointer;
        for (int j = 0; j < 4; j++)
            musicHeader_E3808->str_8.track_10[i].stub_4[j] =
                shadow_str_E3808_music_header->str_8.track_10[i].stub_4[j];
        musicHeader_E3808->str_8.track_10[i].xmiSize_8 =
            shadow_str_E3808_music_header->str_8.track_10[i].xmiSize_8;
        musicHeader_E3808->str_8.track_10[i].word_12 =
            shadow_str_E3808_music_header->str_8.track_10[i].word_12;
        for (int j = 0; j < 18; j++)
            musicHeader_E3808->str_8.track_10[i].filename_14[j] =
                shadow_str_E3808_music_header->str_8.track_10[i].filename_14[j];
    }
    for (int i = 0; i < 14; i++)
...
void sub_8F100_sound_proc19(uint32_t flags, __int16 index, int volume, int volumePan, unsigned __int16 playRate, char loopCount, unsigned __int8 playType)      //270100
{
    ...
    case 2:
        {
            ...
            for (int j = 0; j < x_DWORD_180B4C_end_sound_buffer3_endindex; j++) {
                if (SoundBuffer3_180750[j]->flags_14 == flags
                    && SoundBuffer3_180750[j]->vol_scale_18[0][0] == index
                    && AilSampleStatus_94010(SoundBuffer3_180750[j]) != 2) {
    ...
    case 3:
        {
            ...
            for (int l = 0; l < x_DWORD_180B4C_end_sound_buffer3_endindex; l++) {
                if (SoundBuffer3_180750[l]->flags_14 == flags
                    && SoundBuffer3_180750[l]->vol_scale_18[0][0] == index) {
    ...
    //Logger->info("sub_8F100_sound_proc19 id {} index {}  sz {}  {}", (*soundBuffer1)->id, index, soundIndex_E37A0->str_8.wavs_10[index].wavSize_8, soundIndex_E37A0->str_8.wavs_10[index].filename_14);
...
void sub_8F420_sound_proc20(int a1, __int16 a2) //270420
{
    if (soundAble_E3798 && soundActive_E3799) {
        for (int i = 0; i < x_DWORD_180B4C_end_sound_buffer3_endindex; i++) {
            if (SoundBuffer3_180750[i]->flags_14 == a1
                && SoundBuffer3_180750[i]->vol_scale_18[0][0] == a2
                && AilSampleStatus_94010(SoundBuffer3_180750[i]) != 2) {
...
void sub_8F710_sound_proc21(int flags, __int16 index, int loopCount, unsigned __int8 initTimers, char volScale) //270710
{
    if (soundAble_E3798 && soundActive_E3799 && index <= indexLoadedSound_180B50) {
        for (int i = 0; i < x_DWORD_180B4C_end_sound_buffer3_endindex; i++) {
            if (SoundBuffer3_180750[i]->flags_14 == flags
                && SoundBuffer3_180750[i]->vol_scale_18[0][0] == index
                && AilSampleStatus_94010(SoundBuffer3_180750[i]) != 2) {
                if (loopCount > 127)
                    loopCount = 127;
...
{
    if (pcSpeakerSoundDev_E3819 && musicAble_E37FC && musicActive_E37FD
        && songCurrentlyPlaying_E3802 && AilSequenceStatus_96170(m_hSequence) != 2
        && x_BYTE_E3817 != a1) {
        if (x_BYTE_E3818) ;     // sub_92DC0_AIL_release_timer_handle(x_DWORD_180C80);
        x_BYTE_E3817 = a1;
...
uint8_t get_sample_ptr(const uint8_t index, uint8_t **data, int32_t *len)
{
    ...
    *len = soundIndex_E37A0->str_8.wavs_10[index].wavSize_8;
    *data = soundIndex_E37A0->str_8.wavs_10[index].wavData_0;
